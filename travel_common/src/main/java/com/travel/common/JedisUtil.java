...
public class JedisUtil {
    ...
    static {
        ...
        //这些配置文件在哪配置的？在当前模块下的resoucer中的config.properties中
    ...
    public static void saveChengDuJedis(String line) throws ParseException {
        ...
        if (line.startsWith("end") && line.contains(",")) {
            ...
            //list左添加：lpush 列表名,list值
//            为list中一个订单末尾加上end
            jedis.lpush(Constants.CITY_CODE_CHENG_DU + "_" + orderId, "end");
            //在set集合中删除值a：srem 列表名 a
            //发现了结束标识，将订单从实时订单列表里面移除掉
            // web页面展示轨迹监控:从redis的Constants.REALTIME_ORDERS列表获取数据。实时中移除，web页面也就没了
            ...
            String lng = split[3];
            String lat = split[4];
            //1.存入实时订单单号 到redis的Set集合，自动对相同的订单id进行去重
            ...
            if(StringUtils.isNotEmpty(hget)){
              ...
              //  System.out.println(parseOrder.toString());
                ...
                if(Long.parseLong(timestamp) * 1000 > parseOrder.getEndTime()){
                    ...
                    //当前数据时间比redis当中的开始时间更小，需要更新开始时间和开始经纬度。
                    //什么时候才比redis中的时间小? 感觉这不会执行
//                由于数据延迟上报导致的数据乱序如何解决？
