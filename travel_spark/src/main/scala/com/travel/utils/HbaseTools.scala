...
object HbaseTools extends Logging with Serializable {
 ...
 /* def rowKeyByMD5(column: String*): Array[Byte] = {
  ...
  def putMapData(conn:Connection, tableName: String, rowKey:String, mapData:Map[String , Any]) = {
    ...
    try{
//      再次获取rowkey
//      TODO 预分区0001-0007，rowkey设计：md5加密，最后组成16位
...
//  val connection = ConnectionFactory.createConnection(configuration)
  ...
  def saveToHBaseAndRedis(connection:Connection,jedis:Jedis, eachLine: ConsumerRecord[String, String]): ConsumerRecord[String,String] = {
    ...
    //18901e5f392c5ad98d24c296dcb0afe4,0003a1ceaf2979d0bdeff58da7665a41,1475976808,104.05355,30.70324
    if(line.split(",").size > 4){
//      在成都数据文件中，一个订单结束会有end开头的一行
        //TODO 第一步：成都数据，不是end的数据行，都保存到hbase里面去
...
//        val strings: Array[String] = line.split(",")
      ...
      //TODO 第二步：保存经纬度信息到redis里面去
        //成都的数据5个字段，所以这里发送的是全部的数据
      if(line.split(",").size == 5 || line.contains("end")){
        JedisUtil.saveChengDuJedis(line)
      }
      //无论如何，成都数据都需要往下传递
    }else{
      //TODO 第三步：保存海口数据到hbase
      /**
    ...
    //ssc, kafkaParams, topics, group, "(.*)gps_topic"
...
//      创建表
    ...
    //遍历kafka的topic
    //根据rowkey获取偏移量表中数据,rowkey设计为group:topic
    for(eachTopic <- topics){
//      偏移量表rowkey
      val rowkey = new Get((group+":"+eachTopic).getBytes())
//      获取偏移量表一行的数据
      val result: Result = table.get(rowkey)
      //获取一个个cell单元格，也就是每个分区partition的offset,
      val cells: Array[Cell] = result.rawCells()
      //遍历所有单元格,获取每个分区的offset
      for(result <- cells){
        //cloneQualifier方法：根据单元格取列名  group:topic:partition
...
//    sparkCore读取hbase中的数据
    ...
    val haiKouOrderRdd: RDD[HaiKouOrder] = rddResult.mapPartitions(eachPartition => {
      val orders: Iterator[HaiKouOrder] = eachPartition.map(eachResult => {
        val value: Result = eachResult._2//TableInputFormat的v类型，我们真正要的在hbase中存储的数据
  ...
  //就是一个hbase的put操作，更新hbase中的offset值
